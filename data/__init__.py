...
def create_dataloader(dataset,
    ...
    if phase == 'train':
        ...
        dataloader_args = dict(
            ...
            drop_last=True)
        if sampler is None:
            dataloader_args['shuffle'] = True              #sampler: 决定数据采样顺序的对象；如果sampler为none，在每个epoch开始时打乱数据的顺序
        dataloader_args['worker_init_fn'] = partial(
            worker_init_fn, num_workers=num_workers, rank=rank,
            seed=seed) if seed is not None else None
        if seed is not None:
            dataloader_args['generator'] = torch.Generator()
            dataloader_args['generator'].manual_seed(seed + rank)
